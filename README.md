...
**TMDB.** `TmdbClient` owns the bearer token, maps TMDB's payload to the app's `MovieSummary` shape, caches identical searches for 60 seconds (per running instance — on Vercel that means per warm function), and retries once on a 429 using `Retry-After`. Movie details are snapshotted into the watchlist row at add time, so rendering your list never calls TMDB.

**Auth.** Passwords are hashed with bcrypt (cost 12). Login always runs a hash comparison even for unknown usernames so the two cases take the same time and return the same message. Sessions are regenerated on login (no fixation) and stored in Postgres, so they survive server restarts and cold starts. `/api/auth/*` is rate-limited (in memory, so per function instance on Vercel — enough to blunt casual brute-forcing at this scale; a shared store would be the next step).
